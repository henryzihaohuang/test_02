...
RSpec.describe Candidate, type: :model do
  ...
  let(:orlando_resident) { FactoryBot.create(:candidate, :orlando_resident) }
  ...
  describe 'experience relationship methods' do 
    ...
    describe '#has_disability?' do 
      let(:non_disabled_candidate) { FactoryBot.create(:candidate) }
      let(:disabled_candidate_from_bio) { FactoryBot.create(:candidate, bio: 'neurodiverse student') }
      let(:non_disabled_candidate_with_case_sensitive_disability) { FactoryBot.create(:candidate, bio: 'I like to add') }
      let(:disabled_candidate_with_case_sensitive_disability) { FactoryBot.create(:candidate, bio: "I have ADD") }

      context 'with a disability keyword in the description of an experience' do 
        subject { non_disabled_candidate.has_disability? }
        it { is_expected.to eq(false) }
      end

      context 'with a non matched case sensitive keyword in a candidate bio' do 
        subject { non_disabled_candidate_with_case_sensitive_disability.has_disability? }
        it { is_expected.to eq(false) }
      end

      context 'with a matched case sensitive keyword in a candidate bio' do 
        subject { disabled_candidate_with_case_sensitive_disability.has_disability? }
        it { is_expected.to eq(true) }
      end
      ...
      context 'with a disability keyword in the title of an experience' do 
        before do 
          FactoryBot.create(
            ...
            candidate_id: non_disabled_candidate.id
          )
        end
