class ImportCoresignalUserFileJob < ApplicationJob
  queue_as :default
  ...
  def perform(filename)
    ...
    row_stream(ungzip_stream(decompressor, file_stream(client, filename))).lazy.each do |row|
      candidate_json = Oj.load(row.gsub(",\n", ''))
      ...
      educations = candidate_json['member_education_collection'].collect do |education_json|
        if education_json['deleted'] != 1
          ...
          {
            candidate_uid: candidate_json['id'],
    ...
    results = Candidate.upsert_all(all_candidates, unique_by: :uid, returning: [:id, :uid])

    uid_to_id = results.rows.reduce({}) {|acc, row| acc[row[1]] = row[0]; acc }

    all_educations = all_educations.flatten.collect do |education|
      education[:candidate_id] = uid_to_id[education[:candidate_uid]]
      education.delete(:candidate_uid)

      education
    end

    Education.upsert_all(all_educations, unique_by: :uid)

    all_experiences = all_experiences.flatten.collect do |experience|
      experience[:candidate_id] = uid_to_id[experience[:candidate_uid]]
      experience.delete(:candidate_uid)
